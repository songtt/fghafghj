...
$shi = date('H');
//日期目录
$data_str = substr($fen,0,strlen($fen)-1);
if($data_str == '0'){
    if($shi == '00'){
        $shi = '23';
        $daylog = date('Ymd',strtotime("-1 day"));
    }else{
        $shi = $shi -1;
        if($shi < 10){
            $shi = '0'.$shi;
    ...
    $data_str = 5;
    //若是晚上00点05执行的脚本 则切至昨天的redis库   12-28上线所以12-29 00:05点切默认库
    if($shi.'-'.$data_str == '23-5'){
        switch($week){
        case 1:
            $redis->select(7);
            break;
    ...
    $data_str = substr($fen,0,strlen($fen)-1) - 1;
}
$datelog = $shi.'-'.$data_str;
// $canshu = $argv[1];
//文件名字
$file_name = "v".$datelog.".log";
    ...
    foreach ($file_arr as $key => $value) {
        ...
        if(!empty($value)){
            ...
            if(!empty($array[$x][0])){
                ...
                $array[$x][0] = !isset($array[$x][0])?0:$array[$x][0];
                $array[$x][1] = !isset($array[$x][1])?0:$array[$x][1];
                $array[$x][2] = !isset($array[$x][2])?0:$array[$x][2];
                $array[$x][3] = !isset($array[$x][3])?0:$array[$x][3];
                $array[$x][4] = !isset($array[$x][4])?0:$array[$x][4];
                $array[$x][5] = !isset($array[$x][5])?0:$array[$x][5];
                $array[$x][6] = !isset($array[$x][6])?0:$array[$x][6];
                $array[$x][7] = !isset($array[$x][7])?0:$array[$x][7];
                $array[$x][8] = !isset($array[$x][8])?0:$array[$x][8];
                $array[$x][9] = !isset($array[$x][9])?0:$array[$x][9];
                $array[$x][10] = !isset($array[$x][10])?0:$array[$x][10];
                $array[$x][11] = !isset($array[$x][11])?0:$array[$x][11];
                $array[$x][12] = !isset($array[$x][12])?0:$array[$x][12];
                $array[$x][13] = !isset($array[$x][13])?0:$array[$x][13];
                $array[$x][14] = !isset($array[$x][14])?0:$array[$x][14];
                $array[$x][15] = !isset($array[$x][15])?0:$array[$x][15];
                $array[$x][16] = !isset($array[$x][16])?0:$array[$x][16];
                $array[$x][17] = !isset($array[$x][17])?0:$array[$x][17];
                $array[$x][18] = !isset($array[$x][18])?0:$array[$x][18];
                $array[$x][19] = !isset($array[$x][19])?0:$array[$x][19];
                $array[$x][20] = !isset($array[$x][20])?0:$array[$x][20];
                ...
                $res[$i]['sumprofit'] = substr($array[$x][6],stripos($array[$x][6],'=')+1);
                $res[$i]['sumpay'] = substr($array[$x][7],stripos($array[$x][7],'=')+1);
                $res[$i]['sumadvpay'] = substr($array[$x][8],stripos($array[$x][8],'=')+1);
                $res[$i]['uv_web'] = substr($array[$x][9],stripos($array[$x][9],'=')+1);
                // echo memory_get_usage(),'---333--'."\n";
                $res[$i]['ui_web'] =  substr($array[$x][10],stripos($array[$x][10],'=')+1);
                $res[$i]['web_deduction'] =  substr($array[$x][11],stripos($array[$x][11],'=')+1);
                $res[$i]['adv_deduction']= substr($array[$x][12],stripos($array[$x][12],'=')+1);
                $res[$i]['web_num']= substr($array[$x][13],stripos($array[$x][13],'=')+1);
                $res[$i]['adv_num']= substr($array[$x][14],stripos($array[$x][14],'=')+1);
                $res[$i]['day']= substr($array[$x][15],stripos($array[$x][15],'=')+1);
                $res[$i]['adv_id']= substr($array[$x][16],stripos($array[$x][16],'=')+1);
                $res[$i]['plan_type']= substr($array[$x][17],stripos($array[$x][17],'=')+1);
                $res[$i]['tc_id']= substr($array[$x][18],stripos($array[$x][18],'=')+1);
                $res[$i]['tpl_id']= substr($array[$x][19],stripos($array[$x][19],'=')+1);
                $res[$i]['click_num']= substr($array[$x][20],stripos($array[$x][20],'=')+1);
                ...
                if(!empty($res[$i]['ad_id'])&&!empty($res[$i]['adz_id'])&&!empty($res[$i]['uid'])&&!empty($res[$i]['pid'])&&!empty($res[$i]['adv_id'])&&!empty($res[$i]['plan_type'])&&!empty($res[$i]['tpl_id']))
                {
                    ...
                    // $var = $redis->HMGET($name,array('click_num'));
                    ...
                    $final_res[$name]['web_click_num'] = !isset($final_res[$name]['web_click_num'])?0:$final_res[$name]['web_click_num'];//计划下排重点击
                    $final_res[$name]['adz_click_num'] = !isset($final_res[$name]['adz_click_num'])?0:$final_res[$name]['adz_click_num'];//计划下排重点击
                    $final_res[$name]['click_num'] = !isset($final_res[$name]['click_num'])?0:$final_res[$name]['click_num'];//计划下排重点击
                    ...
                    // $final_res[$name]['click_num'] += $var['click_num'];
