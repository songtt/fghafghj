...
if($ispcflag==1){
    ...
    try {
        $sql = ' INSERT INTO lz_area_ip_stats (uid,ip,area,model,stime) value(?,?,?,?,?) ';
        $advUpdate = $savePdo->prepare($sql);
        $advUpdate->execute(array($uid,$user_IP,$area,$model,$date));
        $savePdo = null;
    } catch (Exception $e) {
        echo '需要在手机端打开.';
    }
...
$userNum = $savePdo->prepare("SELECT * FROM lz_area_ip_stats WHERE uid = ? and area = ? and model = ? and status =3");
$userNum->execute(array($uid, $area,$model));
$userNum_arr = $userNum->fetchAll();

if(!empty($userNum_arr)){
    $updata_sql = $savePdo->prepare("UPDATE lz_area_ip_stats set num=num+1,stime = $date WHERE uid = ? and area = ? and model = ? and status = 3");
    $updata_sql->execute(array($uid, $area, $model));
}else{

    $sql = ' INSERT INTO lz_area_ip_stats (uid,ip,area,model,stime,num,status) value(?,?,?,?,?,?,?) ';
    $advUpdate = $savePdo->prepare($sql);
