...
try{
    ...
    $pdo = new PDO('mysql:host=101.201.107.95;dbname=lezun_copy3;port=3306','root','xya197a3321');
...
//统计计费的函数****************************************************************************************************************
chapv($list,$pdo,$sredis);

//统计计费的函数****************************************************************************************************************
function chapv($list,$pdo,$sredis)
{
    //得到该次展示的单价和扣量
    ...
    //查看计费次数
    $uiNum = getUiNum($list,$sredis);
    //更新实时ip表
    $ipNumber = updateRealtimeip($list,$uiNum,$sredis);
    //更新浏览器表
    $uvNumber = updateBrowser($list,$sredis);
    //按广告位算的独立ip  separate
    $separateip = $sredis->handler()->HMGET('separate-ip-'.$list['adz_id'],array($list['user_ip']));
    if(empty($separateip[$list['user_ip']])){
        //此ip 没有访问此广告位id时存入
        $array_userip = array(
            $list['user_ip'] => 1,
        );
        $sredis->handler()->HMSET('separate-ip-'.$list['adz_id'],$array_userip);
    }

    $dayTime = date('Y-m-d');
    statsUpdate($list,$uiNum,$uvNumber,$ipNumber,$sredis,$dayTime,$pdo);
    $pdo = null;
}
//得到广告价格和扣量
...
// 查看当前uv计费次数
function getUiNum($list,$sredis)
{
    //=======================独立访客计数===============
    $uiNum = $sredis->handler()->HMGET('ui_ip_'.$list['adz_id'],array($list['user_ip']));
...
//CPM计费模式下更新统计表数据
function statsUpdate($list,$uiNum,$uvNumber,$ipNumber,$sredis,$dayTime,$pdo)
{
    ...
    if($list['plan_type'] !='CPC'){
        //用户独立ip访问的次数所统计不同的价钱
            ...
            $sredis->handler()->HINCRBYFLOAT('users-'.$list['planuid'],'adv_money',$list['adv_money']);
            $sredis->handler()->HINCRBYFLOAT('users-'.$list['planuid'],'key_money',$list['adv_money']);
            $adv_money = $sredis->handler()->HMGET('users-'.$list['planuid'],array('adv_money','key_money'));
            $update_advmoney = 10;
            if($adv_money['key_money'] > $update_advmoney){
                ...
                $sredis->handler()->HMSET('users-'.$list['planuid'],$adv_money);
            }
            ...
            //缓存计划限额
            $sredis->handler()->HINCRBYFLOAT('budget-'.$list['pid'].'-'.$dayTime,'budget',$list['adv_money']);
            ...
            //将redis里面的数据同步到数据库
            redis_update($list,$sredis,$dayTime,$pdo);
...
//将redis里面的数据同步到数据库
function redis_update($list,$sredis,$dayTime,$pdo)
{
    //处理计划限额
    $money = $sredis->handler()->HMGET('budget-'.$list['pid'].'-'.$dayTime,array('budget'));
    if($list['budget'] <= $money['budget']){

        if(!empty($list['type'])){

            //当前计划是否为游戏推广计划，查看是否缓存中存在
            $game_pid = $sredis->handler()->HMGET('game-pid-budget',array($list['pid']));
            ...
            if(empty($game_pid[$list['pid']])){
                ...
                $sredis->handler()->HMSET('game-pid-budget',$game_pid_array);
            }else{
                ...
                $sredis->handler()->HMSET('game-pid-budget',$game_pid_initialize);
                //初始化改计划限额
                $game_budget = array(
                    'budget' => $list['adv_money'],
                );
                $sredis->handler()->HMSET('budget-'.$list['pid'].'-'.$dayTime,$game_budget);
            }
        }
        $money = $sredis->handler()->HMGET('budget-'.$list['pid'].'-'.$dayTime,array('budget'));
...
//判断实时ip信息
function updateRealtimeip($list,$uiNum,$sredis)
{
    if(!empty($uiNum)){
        //数据报表独立访客+1
        $uiNumber = 0;
        $sredis->handler()->HINCRBY('ui_ip_'.$list['adz_id'],$list['user_ip'],1);
        ...
        $sredis->handler()->HMSET('ui_ip_'.$list['adz_id'],$ip_arr);
    }
    return $uiNumber;
}

//判断有无,添加或修改浏览器表数据
function updateBrowser($list,$sredis)
{
    //得到是否有这个ip
    $havaIp = $sredis->handler()->SISMEMBER('uv_'.$list['adz_id'],$list['base_cookies']);
    ...
    if(empty($havaIp)){
        $uvNumber = 1;
        //存各个站长独立ip
        $sredis->handler()->SADD('uv_'.$list['adz_id'],$list['base_cookies']);
    }else{
        $uvNumber = 0;
    }
    return $uvNumber;
