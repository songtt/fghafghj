...
$blogid = substr($urlnum[0], 8);
$pid = substr($urlnum[1], 4);
$uid = substr($urlnum[2], 4);
$tc_id = substr($urlnum[3], 6);
$tpl_id = substr($urlnum[4], 7);
$plan_type = substr($urlnum[5], 10);
$planuid = substr($urlnum[6], 8);
$site_id = substr($urlnum[7], 8);
$user_ip = substr($urlnum[8], 8);
$base_cookies = substr($urlnum[9], 13);
$browser = substr($urlnum[10], 8);
$ver = substr($urlnum[11], 4);
$kernel = substr($urlnum[12], 7);
$modle_name = substr($urlnum[13], 11);
$system_version = substr($urlnum[14], 15);

if(empty($blogid) || empty($_GET['id']) || empty($pid) || empty($uid)){ exit; }

//正则截取GET提交的id(只留取数字)
$id = getIdCut($_GET['id']);
$blog_id = getIdCut($blogid);
$pid = getIdCut($pid);
$uid = getIdCut($uid);
...
function statsInsert($list,$pdo,$uiNum,$uvNumber,$ipNumber,$self_adv_id,$redis,$dayTime)
{
    ...
    $insert = $pdo->prepare("INSERT INTO lz_stats (uid,pid,ad_id,tc_id,adv_id,adtpl_id,site_id,adz_id,sumprofit,sumpay,sumadvpay,
            views,day,web_deduction,adv_deduction,web_num,adv_num,unique_visitor,unique_ip,uv_web,ui_web,ui_adzone,plan_type,heavy_click_num,ctime)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
...
function statsUpdate($list,$pdo,$uiNum,$uvNumber,$ipNumber,$self_adv_id,$redis,$dayTime)
{
    ...
    // $redistwo = new Redis();
    // $redistwo->connect('127.0.0.1', 6379);
    // $redistwo->auth('cyredis'); //设置密码
...
function redis_update($list,$pdo,$redis,$dayTime)
{
    ...
    // // $redis_num = 151;
    // // $redis_num_e = 148;
    // $redis_num = $list['adz_id'] % 2 == 0 ? 151 : 148;
    // $redis_num_e = $list['adz_id'] % 2 == 0 ? 148 : 149;
    // if($h >= 0 && $h <= 1){
    //     $redis_num = $list['adz_id'] % 2 == 0 ? 151 : 148;
    // }else {
    //     //$redis_num = substr($list['adz_id'],-2) + 50;
    //     // $redis_num = $list['adz_id'] % 2 == 0 ? 231 : 232;
    //     $redis_num = 231;
    //     $redis_num_e = 232;
    //     if($views['views']<=3000){
    //         // $redis_num = $list['adz_id'] % 2 == 0 ? 10 : 11;
    //         $redis_num = 10;
    //         $redis_num_e = 11;
    //     }
    //     if($views['views']>3000&&$views['views']<=20000){
    //         // $redis_num = $list['adz_id'] % 2 == 0 ? 50 : 51;
    //         $redis_num = 50;
    //         $redis_num_e = 51;
    //     }
    //     if($views['views']>20000&&$views['views']<=90000){
    //         // $redis_num = $list['adz_id'] % 2 == 0 ? 80 : 81;
    //         $redis_num = 80;
    //         $redis_num_e = 81;
    //     }
    // if($views['views']>100000&&$views['views']<=800000){
    //     // $redis_num = $list['adz_id'] % 2 == 0 ? 151 : 150;
    //     $redis_num = 251;
    //     $redis_num_e = 252;
    // }
//     if($views['views']>800000){
//         // $redis_num = $list['adz_id'] % 2 == 0 ? 151 : 150;
//         $redis_num = 500;
//         $redis_num_e = 501;
//     }
//     // }
//     // $redis_num = $list['adz_id'] % 2 == 0 ? 151 : 148;
//     if($views['views'] % $redis_num == 0 || $views['views'] % $redis_num_e == 0){
//         //计划排重点击
...
//         //按站长算 排重点击数
//         $web_click_uid = $redis->handler()->HLEN('web-click-uid-'.$list['uid']);
...
//         //按广告位算 排重点击数
//         $adz_click_id = $redis->handler()->HLEN('adz-click-id-'.$list['adz_id']);
...
//         //更新数据报表的数据
//         $update = $pdo->prepare("UPDATE lz_stats SET sumprofit=?,sumpay=?,sumadvpay=?,views=?,click_num=?,uv_web=?,ui_web=?,ui_adzone=?,web_deduction=?,adv_deduction=?,web_num=?,adv_num=?,web_click_num=?,adz_click_num=?,heavy_click_num=? WHERE adz_id=? AND ad_id=? AND site_id=? AND day=?");

//         $update->execute(array($views['sumprofit'],$views['sumpay'],$views['sumadvpay'],$views['views'],$views['click_num'],$views['uv_web'],$views['ui_web'],$adzone_ip,$views['web_deduction'],$views['adv_deduction'],$views['web_num'],$views['adv_num'],$web_click_num,$adz_click_num,$heavy_num,$list['adz_id'],$list['ad_id'],$list['site_id'],$dayTime));
