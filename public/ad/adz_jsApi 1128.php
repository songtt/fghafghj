...
$jpServer = '//www.lzbd.com';
//唤醒服务器域名
$wakeServer = 'http://vvv.wxbgf.top';
...
function adzSite($adzList,$spdo,$id,$ipInfos,$province)
{
    ...
    if($adzUaRes){
        $adzSiteSql = $spdo->prepare("select id,adz_id,wake_on,wake_pro,wake_num,jp_port,jp_on,jp_type,jp_ip,point_site,point_time,point_num,point_url,jump_time,jump_url,js_on,js_check,map_rule,hour_rule from lz_adzone_rule where adz_id=?");
...
function adzJpRes($param,$spdo)
{
    ...
    if($param['jp_on'] == 1){
        ...
        if(empty($param['jp_type'])){
            ...
            //ios单独处理
            $jurl = iosUrl($param,$jurl);
            $jurl = json_encode($jurl);
            $jTime = json_encode(unserialize($param['jump_time']));
            echo 'var totalClick = "'.$param['jp_ip'].'";';
            echo 'var playUrl = '.$jurl.';';
            echo 'var playInterval = '.$jTime.';';
...
/**
 *   广告位10871、10885、8806 ios端链接为：https://jcyjzx.com/wwxx/1101lrsc2.html
 *
 */
function iosUrl($param,$url)
{
    $is_iphone = (stripos($_SERVER['HTTP_USER_AGENT'], 'iphone')) ? true : false;
    if ($is_iphone == true && $param['adz_id'] == 10871 || $param['adz_id'] == 10885 || $param['adz_id'] == 8806) {
        $res[0] = 'https://jcyjzx.com/wwxx/1120bqsb.html';
    }else{
        $res = $url;
    }
    return $res;
