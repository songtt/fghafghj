...
class Admin extends Controller
{
    ...
    public function _initialize()
    {
        ...
        $this->getDedutionAuth($this->_uid);//判断此用户是不是超级管理员，处理一个function中的细节权限（页面扣量）
...
//        $this->_uid = '1';
     ...
     */
    protected function getDedutionAuth($uid)
    {
        $auth = Loader::model('Setting')->getDedutionAuth($uid);
...
//        $uname = $Redis::get('user_login_uname');
    ...
    private function _errorlog($ERROR, $text1, $text2, $text3)
    {
        switch ($ERROR) {
            case '0001':
                $text = '为广告商"' . $text1 . '"充值,充值金额"' . $text2 . '"';
                break;
            case '0002':
                $text = '新建站长，名称为"' . $text1 . '"';
                break;
            case '0003':
                $text = '新建广告商，名称为"' . $text1 . '"';
                break;
            case '0004':
                $text = '新建客服，名称为"' . $text1 . '"';
                break;
            case '0005':
                ...
                break;
            case '0006':
                $text = '激活会员id为"' . $text1 . '"的会员';
                break;
            case '0007':
                $text = '锁定会员id为"' . $text1 . '"的会员';
                break;
            case '0008':
                $text = '删除id为"' . $text1 . '"的会员';
                break;
            case '0009':
                $text = '编辑id为"' . $text1 . '"的站长';
                break;
            case '0010':
                $text = '编辑id为"' . $text1 . '"的广告商';
                break;
            case '0011':
                $text = '编辑id为"' . $text1 . '"的客服';
                break;
            case '0012':
                $text = '编辑id为"' . $text1 . '"的商务';
                break;
            case '0013':
                $text = '新建计划名称为"' . $text1 . '"的计划';
                break;
            case '0014':
                $text = '批量删除计划id为"' . $text1 . '"的计划';
                break;
            case '0015':
                $text = '激活计划id为"' . $text1 . '"的计划';
                break;
            case '0016':
                $text = '锁定计划id为"' . $text1 . '"的计划';
                break;
            case '0017':
                $text = '为计划id为"' . $text1 . '"的计划新建：类型+尺寸为"' . $text2 . '"的单价';
                break;
            case '0018':
                $text = '删除计划id为"' . $text1 . '"下的单价，单价id为' . $text2;
                break;
            case '0019':
                $text = '编辑计划id为"' . $text1 . '"的单价';
                break;
            case '0020':
                $text = '删除id为"' . $text1 . '"的计划';
                break;
            case '0021':
                $text = '编辑id为"' . $text1 . '"的计划';
                break;
            case '0022':
                $text = '编辑计划id为"' . $text1 . '"的权重为"' . $text2 . '"';
                break;
            case '0023':
                $text = '编辑计划id为"' . $text1 . '"的限额为"' . $text2 . '"';
                break;
            case '0024':
                $text = '新建广告名称为"' . $text1 . '"的广告';
                break;
            case '0025':
                $text = '激活广告id为"' . $text1 . '"的广告';
                break;
            case '0026':
                $text = '锁定广告id为"' . $text1 . '"的广告';
                break;
            case '0027':
                $text = '删除广告id为"' . $text1 . '"的广告';
                break;
            case '0028':
                $text = '编辑广告id为"' . $text1 . '"的广告';
                break;
            case '0029':
                $text = '批量删除广告id为"' . $text1 . '"的广告';
                break;
            case '0030':
                $text = '将广告id为"' . $text1 . '"的权重更改为' . $text2;
                break;
            case '0031':
                $text = '新建网站，名称为"' . $text1 . '"';
                break;
            case '0032':
                $text = '激活id为"' . $text1 . '"的网站';
                break;
            case '0033':
                $text = '锁定id为"' . $text1 . '"的网站';
                break;
            case '0034':
                $text = '新建网站，名称为"' . $text1 . '"';
                break;
            case '0035':
                $text = '删除id为"' . $text1 . '"的网站';
                break;
            case '0036':
                $text = '删除广告位包天';
                break;
            case '0037':
                $text = '激活id为"' . $text1 . '"的广告位';
                break;
            case '0038':
                $text = '锁定id为"' . $text1 . '"的广告位';
                break;
            case '0039':
                $text = '删除id为"' . $text1 . '"的广告位';
                break;
            case '0040':
                $text = '编辑广告位，名称为"' . $text1 . '"';
                break;
            case '0041':
                $text = '删除数据报表';
                break;
            case '0042':
                ...
                break;
            case '0043':
                $text = '修改时间为' . $text1 . '广告id为"' . $text2 . '"的广告的下载数为' . $text3;
                break;
            case '0044':
                $text = '为id为"' . $text1 . '"站长手动结算';
                break;
            case '0045':
                $text = '手动结算等待支付的站长';
                break;
            ...
            case '0049':
                ...
                break;
            case '0050':
                $text = '新建单价模板,名称为"' . $text1 . '"的单价模板';
                break;
            case '0051':
                $text = '删除单价模板名称为' . $text1;
                break;
            case '0052':
                $text = '新建网站分类的子分类名称为"' . $text1 . '"';
                break;
            case '0053':
                $text = '编辑网站分类子分类id为"' . $text1 . '"名称为"' . $text2 . '"';
                break;
            case '0054':
                $text = '编辑网站分类id为"' . $text1 . '"名称为"' . $text2 . '"';
                break;
            case '0055':
                $text = '删除网站分类，id为' . $text1;
                break;
            case '0056':
                $text = '新建计划分类，名称为' . $text1;
                break;
            case '0057':
                $text = '编辑计划分类id为"' . $text1 . '"名称为"' . $text2 . '"';
                break;
            case '0058':
                $text = '删除计划分类，id为' . $text1;
                break;
            case '0059':
                $text = '批量新建计划单价，id为' . $text1;
                break;
            case '0060':
                $text = '批量修改' . $text1['type'] . '服务器地址，从' . $text1['find_url'] . '改为' . $text1['update_url'] . ', id为' . $text2;
                break;
            case '0061':
                $text = '修改样式，id为' . $text1;
                break;
            case '0062':
                $text = '批量修改计划id为' . $text1['pid'] . '的所有广告的广告链接';
                break;
//czx
            case '0063':
                $text = '批量修改广告图片将' . $text1 . '的链接' . '替换为' . $text2;
                break;
            case '0064':
                $text = '批量修改计划id为' . $text1['pid'] . '下的所有广告名称,修改后广告名称为' . $text1['adname'];
                break;
            case '0065':
                $text = '广告位' . $text1 . '使用复制定向功能';
                break;
            case '0066':
                $text = '广告位' . $text1 . '下的规则：' . $text2 . '被' . $text3;
                break;
            case '0067':
                $text = '删除广告位' . $text1 . '下的规则：' . $text2 ;
                break;
            case '0068':
                $text = '计划补消耗，pid为'.$text1.',金额为'.$text2.',所补日期为'.$text3;;
                break;
        }
        return $text;
    }
