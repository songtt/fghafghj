...
class Hpage extends Controller
{
    ...
    public function article()
    {
        ...
        if($params['type'] == 1){
            ...
            return $this->fetch('typeone');
        }elseif($params['type'] == 2){
            return $this->fetch('typetwo');
        }elseif($params['type'] == 3){
            return $this->fetch('typethree');
        }else{
            return $this->fetch('typefour');
        }
    ...
    public function register()
    {
        ...
        if(Request::instance()->isPost()) {
            //检验是否有必填信息没填
            if($params['type'] == 1){
                if((empty($params['reg_name'])) || (empty($params['nickname_web'])) || (empty($params['reg_password']))
                    || (empty($params['reg_password_2'])) || (empty($params['contact'])) || (empty($params['tel'])) || (empty($params['qq']))
                    || (empty($params['email']))  || (empty($params['bank_name']))  || (empty($params['account_name']))  || (empty($params['bank_branch']))
                    || (empty($params['bank_card']))){
                    $this->_error($info='必填信息不能为空！');
                }
            }else{
                if((empty($params['reg_name'])) || (empty($params['nickname_adv'])) || (empty($params['reg_password']))
                    || (empty($params['reg_password_2'])) || (empty($params['contact'])) || (empty($params['tel'])) || (empty($params['qq']))
                    || (empty($params['email']))){
                    $this->_error($info='必填信息不能为空！!');
                }
            ...
            //判断URL是否是 客户或商务的发展链接(uid和type必须同时有值，否则值为空)
            if(empty($params['type']) || empty($params['uid'])){
                $params['type'] = '';
                $params['uid'] = '';
            }
            //获取所有的客服
            $res['customer'] = Loader::model('Index')->getusers(3);
            //获取所有的商务
            $res['bussess'] = Loader::model('Index')->getusers(4);
...
//                }
                ...
                if($type==1){
                    ...
                    $getIP =$this-> _getIp();
                    import("extend.org.monipdb");
                    $IpSearch = new IpSearch(__DIR__.'./../../../extend/org/qqzeng-ip-utf8.dat');
                    $ipInfos = $this->_getIpLookup($getIP,$IpSearch);
                    ...
                    $log_test_str = '站长用户名：'.$uname.','.'IP:'.$getIP.','.'地域'.$ipInfos[2].$ipInfos[3].','.'登录时间'.date('Y-m-d H:i:s').' 登录成功'."\r\n";
                    ...
                    $log_test_file =  __DIR__.'./../../../public/test/loginlogs.log';
                    $log_test_str = '客服：'.$uname.','.'IP:'.$getIP.','.'地域'.$ipInfos[2].$ipInfos[3].','.'登录时间'.date('Y-m-d H:i:s').'客服登录成功'."\r\n";
                    $this->writeFile($log_test_file,$log_test_str);
    ...
    private function _getUsers($params)
    {
        ...
        $date = array(
            'type' => $params['type'],
            'nickname' => empty($params['nickname_adv']) ? $params['nickname_web'] : $params['nickname_adv'],
            'username' => $params['reg_name'],
            'password' => $params['password'],
            ...
            'serviceid' => empty($params['serviceid_web']) ? $params['serviceid_adv'] : $params['serviceid_web'],
            'status' => 2,
            'regip' => $ip,
        );
        if(!empty($params['uid'])){
     ...
     * 根据用户ip获取地理位置
     */
    private function _getIpLookup($ip = '',$IpSearch)
    {
        header("Content-Type: text/html;charset=utf-8");
        $res = $IpSearch->get($ip);
        if(!empty($res)){
